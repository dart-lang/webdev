...
void main() {
  test('build script successfully generates client.js and version.dart', () async {
    final result = await Process.run('dart', ['run', 'tool/build.dart']);
    
    expect(result.exitCode, 0, reason: 'Build script failed: ${result.stdout}\n${result.stderr}');
    ...
    expect(versionFile.readAsStringSync(), contains('const packageVersion ='));
  });
}
