  ...
  Future<int?> _lookUpId() async {
    ...
    while (await _contexts.hasNext.timeout(
      ...
      try {
        ...
        final resultObj = result.result?['result'] as Map<String, dynamic>?;
  ...
  RemoteDebuggerExecutionContext(this._id, this._remoteDebugger) {
    ...
    _remoteDebugger
        .eventStream('Runtime.executionContextCreated', (e) {
          ...
          // If we cannot detect or parse the context ID, add `null` to
          // the stream to indicate an error context - those will be
          // skipped when trying to find the dart context, with a warning.
          final context = e.params?['context'] as Map<String, dynamic>?;
