...
Logger _logger = Logger('AssetProxyHandler');
...
Handler assetProxyHandler(url, {http.Client? client, String? proxyName}) {
  Uri uri;
  if (url is String) {
    uri = Uri.parse(url);
  } else if (url is Uri) {
  ...
  return (serverRequest) async {
    try {
      ...
      unawaited(serverRequest
        ...
        _logger.finest('Error reading request: $e, $s');
        clientRequest.sink.addError(e, s);
      ...
      _logger.finest('Error sending request: $e:$s');
      rethrow;
    }
  };
}
